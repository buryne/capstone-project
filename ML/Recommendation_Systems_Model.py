...
def rekomendasi_tempat_wisata(Place_Name, cosine_sim=cosine_sim):
    ...
    similar_places_indices = [i for i in similar_places_indices if df_wisata['Place_Name'].iloc[i] != Place_Name]
...
# Train the model
model.fit(tfidf_matrix.toarray(), tfidf_matrix.toarray(), epochs=10, batch_size=32, validation_split=0.2, callbacks=[callbacks])
